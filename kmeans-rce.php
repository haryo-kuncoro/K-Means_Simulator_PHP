...
class KMeans {
    ...
    public function __construct(array $data, int $k, int $maxIterations = 100) {
        ...
        $this->iterationClusterCounts = []; // Inisialisasi properti baru
    }
    ...
    private function initializeCentroidsRCE() {
        // Handle case where data might be too small for k
        ...
        for ($i = 1; $i < $this->k; $i++) {
            ...
            if ($nextCentroidIndex !== -1 && !in_array($this->data[$nextCentroidIndex], $this->centroids)) { // Pastikan tidak memilih centroid yang sama
                $this->centroids[$i] = $this->data[$nextCentroidIndex];
            } else {
                // Fallback jika tidak ada titik baru yang optimal, pilih acak yang belum ada
                $availableIndices = array_diff(array_keys($this->data), array_keys($this->centroids)); // Hindari duplikat
                if (!empty($availableIndices)) {
                    $this->centroids[$i] = $this->data[array_rand($availableIndices)];
                } else {
                    // Semua data sudah menjadi centroid atau tidak ada cukup data unik
                    // Ini skenario yang jarang terjadi jika validasi awal k < count(data) sudah benar
                    break; // Keluar dari loop jika tidak bisa mendapatkan centroid baru
                }
            }
        }
        // Pastikan kita memiliki K centroid, jika tidak, mungkin ada masalah dengan data atau K terlalu besar
        while (count($this->centroids) < $this->k) {
             $this->centroids[] = $this->data[array_rand($this->data)]; // Tambahkan secara acak sampai k tercapai
        }
        $this->centroids = array_slice($this->centroids, 0, $this->k); // Pastikan hanya ada k centroid
    }
    ...
    public function run(): array {
        ...
        for ($i = 0; $i < $this->maxIterations; $i++) {
            ...
            // Store cluster counts for the current iteration
            $this->iterationClusterCounts[$i] = $this->getClusterCounts();

            // HENTIKAN KONDISI EARLY BREAK INI jika ingin selalu 4 iterasi
            // Jika Anda ingin algoritma selalu berjalan sampai maxIterations, abaikan kondisi !hasChanged ini
            // Namun, K-Means tradisional akan berhenti jika tidak ada perubahan
            // Untuk tujuan visualisasi iterasi yang diminta, kita akan tetap melanjutkannya
            // if (!$hasChanged && $i > 0) {
            //     break;
            // }
        }
        ...
        return [
            ...
            'iteration_cluster_counts' => $this->iterationClusterCounts // Tambahkan data ini
        ];
    }
    ...
    public function calculateSilhouetteScore(): float {
        ...
        foreach ($this->data as $dataIndex => $point) {
            ...
            if ($assignedClusterId === -1 || !isset($this->clusters[$assignedClusterId]) || count($this->clusters[$assignedClusterId]) <= 1) {
                $a_i = 0.0; // Jika hanya satu anggota di cluster atau tidak ditugaskan
            } else {
                $a_i = 0.0;
...
<body>
    <div class="container">
        ...
        <h1>Dengan Optimasi Rapid Centroid Estimation (RCE)</h1>
